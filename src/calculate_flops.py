...
def get_model_and_shape(config):
...
def main(args):
    ...
    model, input_shape = get_model_and_shape(config)
    ...
    def sparse_r2conv_handler(m: SparseR2Conv, x, y):
        ...
        dense_flops = dense_r2conv_handler(m.conv, x, y) # Reuse dense handler
        m.total_ops += torch.DoubleTensor([dense_flops * active_ratio])
        return dense_flops * active_ratio
